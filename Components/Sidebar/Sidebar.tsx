...
export default function Sidebar({ extended = false }: { extended?: boolean }) {
    ...
    return (
        <aside className={`absolute trns shadow-lg rounded-lg flex flex-col gap-1 p-4 min-w-1/4 min-h-[80vh] bg-gray-100 right-0 bottom-0 translate-y-full ${extended ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <Link className="w-full trns hover:bg-white p-2 rounded-sm sm:hidden" href="/notifications">Notification</Link>
            <Link className="w-full trns hover:bg-white p-2 rounded-sm sm:hidden" href="/liked-post">Liked</Link>
            <Link className="w-full trns hover:bg-white p-2 rounded-sm sm:hidden" href="/explore">Explore</Link>
